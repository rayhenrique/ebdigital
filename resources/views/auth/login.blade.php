<x-guest-layout>
    ...
    <form method="POST" action="{{ route('login') }}" class="space-y-4" x-data="{ showPassword: false }">
        ...
        <div class="relative flex py-2 items-center">
            ...
            <span class="flex-shrink mx-3 text-[11px] font-semibold text-slate-400 uppercase tracking-wider">Novo por aqui?</span>
            <div class="flex-grow border-t border-slate-200"></div>
        </div>
        ...
        <div>
            <a 
                href="{{ route('register') }}" 
                class="w-full flex items-center justify-center gap-2 px-4 py-3 min-h-[48px] bg-slate-50 hover:bg-slate-100 text-slate-700 font-bold text-xs sm:text-sm rounded-xl border border-slate-200 shadow-sm transition active:scale-[0.98] cursor-pointer"
            >
                ...
                <span>Solicitar Cadastro (Professores e Secretaria)</span>
            </a>
        </div>
    </form>
</x-guest-layout>
